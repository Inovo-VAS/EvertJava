...
//			System.out.println("[" + service + "]" + cInfo + aInfo);
	...
	private void insertWallboardEntry(String serviceID, cAgentInfo agentInfo, cCallInfo callInfo) throws Exception{
		...
		if(!callInfo.agentCalls.isEmpty()){
			...
			for(int rowIndex:callInfo.agentCalls.keySet()){
				...
				if(serviceCallsStatsInfo.get("SERVICELOGINCALLCOUNT").equals("0")){
					Database.executeDBRequest(null,"PMCONSOLREPORTS","INSERT INTO <DBUSER>.WALLBOARD_SERVICEAGENTCALLSSTATUS (ID,SERVICEID,BOUNDTYPE,CALLID,PHONE,LOGIN,TIME,STATUS,LAST_UPDATED) VALUES (:ID ,:SERVICEID ,:BOUNDTYPE ,:CALLID ,:PHONE ,:LOGIN ,:TIME ,:STATUS ,GETDATE())",serviceCallsStatsInfo,null);
				}
				else{
					Database.executeDBRequest(null,"PMCONSOLREPORTS","UPDATE <DBUSER>.WALLBOARD_SERVICEAGENTCALLSSTATUS SET ID=:ID ,SERVICEID=:SERVICEID ,BOUNDTYPE=:BOUNDTYPE ,CALLID=:CALLID ,PHONE=:PHONE ,LOGIN=:LOGIN ,TIME=:TIME ,STATUS=:STATUS ,LAST_UPDATED=GETDATE() WHERE SERVICEID=:SERVICEID AND LOGIN=:LOGIN",serviceCallsStatsInfo,null);
				}
		...
		Database.executeDBRequest(null,"PMCONSOLREPORTS", "DELETE FROM <DBUSER>.WALLBOARD_SERVICEAGENTCALLSSTATUS WHERE SERVICEID = "+callInfo.service+" AND LOGIN NOT IN ("+loginIds+"0)", null,null);

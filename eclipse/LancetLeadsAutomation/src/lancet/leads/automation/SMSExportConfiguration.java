...
public class SMSExportConfiguration extends InovoHTMLPageWidget {
	...
	public void smslayoutsections() throws Exception{
		...
		TreeMap<Integer,ArrayList<String>> smslayoutsectionsset=new TreeMap<Integer, ArrayList<String>>();
		Database.executeDBRequest(smslayoutsectionsset,"LANCETLEADSAUTOMATION", "SELECT ID ,SECTION_TYPE ,SECTION_CONTENT ,SECTION_ORDER ,CREATIONDATE,(SELECT COUNT(*) FROM <DBUSER>.LANCET_SMS_MESSAGE_SECTIONS WHERE SMS_MESSAGE_ID="+this.requestParameter("SMSLAYOUTID")+") AS TOTALSECTIONS FROM <DBUSER>.LANCET_SMS_MESSAGE_SECTIONS WHERE SMS_MESSAGE_ID="+this.requestParameter("SMSLAYOUTID")+" ORDER BY SECTION_ORDER", null,null);
		...
		Database.cleanupDataset(smslayoutsectionsset);
	}
	...
	public void movesection() throws Exception{
		...
		if(smssectionmoveinfo.get("MOVE").equals("UP")){
			if(sectionorder>1&&totalsections>1){
				Database.executeDBRequest(null,"LANCETLEADSAUTOMATION", "UPDATE <DBUSER>.LANCET_SMS_MESSAGE_SECTIONS SET SECTION_ORDER=((SECTION_ORDER)+1) WHERE SMS_MESSAGE_ID=:SMSLAYOUTID AND SECTION_ORDER=((:SECTION_ORDER)-1)", smssectionmoveinfo,null);
				Database.executeDBRequest(null,"LANCETLEADSAUTOMATION", "UPDATE <DBUSER>.LANCET_SMS_MESSAGE_SECTIONS SET SECTION_ORDER=((SECTION_ORDER)-1) WHERE ID=:SECTIONID", smssectionmoveinfo,null);
				changedmove=true;
			}			
		}
	...
	public void deletesection() throws Exception{
		...
		if(sectionorder<=totalsections){
			Database.executeDBRequest(null,"LANCETLEADSAUTOMATION", "DELETE FROM <DBUSER>.LANCET_SMS_MESSAGE_SECTIONS WHERE ID="+smssectionmoveinfo.get("SECTIONID"), null,null);
			while(sectionorder<totalsections){
				Database.executeDBRequest(null,"LANCETLEADSAUTOMATION", "UPDATE <DBUSER>.LANCET_SMS_MESSAGE_SECTIONS SET SECTION_ORDER="+String.valueOf(sectionorder)+" WHERE SMS_MESSAGE_ID=:SMSLAYOUTID AND SECTION_ORDER=("+String.valueOf((sectionorder+1))+")", smssectionmoveinfo,null);
				sectionorder++;
			}
		}
	...
	public void updatesmsmessagedetails() throws Exception{
		...
		if(this.requestParameter("SMS_MESSAGE_NAME").equals("")){
			...
			Database.executeDBRequest(null,"LANCETLEADSAUTOMATION", "SELECT COUNT(*) AS SMS_MESSAGE_COUNT FROM <DBUSER>.LANCET_SMS_MESSAGE WHERE UPPER(SMS_MESSAGE_NAME)=UPPER(:SMS_MESSAGE_NAME) AND ID <> :SMSLAYOUTID", smsmessagedetails,null);
			if(smsmessagedetails.get("SMS_MESSAGE_COUNT").equals("0")){
				Database.executeDBRequest(null,"LANCETLEADSAUTOMATION", "UPDATE <DBUSER>.LANCET_SMS_MESSAGE SET SMS_MESSAGE_NAME=:SMS_MESSAGE_NAME,SMS_MESSAGE_DESCRIPTION=:SMS_MESSAGE_DESCRIPTION WHERE ID=:SMSLAYOUTID", smsmessagedetails,null);
			}
			else{

...
public class FlatFileRequestCallsImporter implements Runnable {
	...
	public void readFileCallRowData(Integer rowindex,ArrayList<String> rowData,ArrayList<String> rowColumns){
		...
		if(serviceid.equals("")){
			this._recordProcessedFailed++;
			try {
				Database.executeDBRequest(null, "FLATFILELEADSIMPORTER", "UPDATE <DBUSER>.[LEADSDATAFILEALLIASREQUESTCONTENT] SET ENTRYHANDLEFLAG=5,ENTRYLASTACTIONDATETIME=GETDATE(),ENTRYSTATUS=1,CALLREQUESTSTATUS=0,ENTRYSERVICEID=0,ENTRYLOADID=0,ENTRYSOURCEID=0 WHERE ID="+Database.rowField(rowColumns, rowData, "ID"), null, null);
			} catch (Exception e) {
		...
		if((loadid=loadid.equals("0")?"":loadid).equals("")||_forcecreatingload){
			...
			if(loadid.equals("")||loadid.equals("0")){
				this._recordProcessedFailed++;
				try {
					Database.executeDBRequest(null, "FLATFILELEADSIMPORTER", "UPDATE <DBUSER>.[LEADSDATAFILEALLIASREQUESTCONTENT] SET ENTRYHANDLEFLAG=6,ENTRYSTATUS=1,ENTRYLASTACTIONDATETIME=GETDATE(),CALLREQUESTSTATUS=0,ENTRYSERVICEID="+serviceid+",ENTRYLOADID=0,ENTRYSOURCEID=0 WHERE ID="+Database.rowField(rowColumns, rowData, "ID"), null, null);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
		...
		if(phonenumfirst.equals("")&&loadrequesttype.equals("1")){
			...
			this._recordProcessedFailed++;
			try {
				Database.executeDBRequest(null, "FLATFILELEADSIMPORTER", "UPDATE <DBUSER>.[LEADSDATAFILEALLIASREQUESTCONTENT] SET ENTRYHANDLEFLAG=7,ENTRYSTATUS=1,ENTRYLASTACTIONDATETIME=GETDATE(),CALLREQUESTSTATUS=0,ENTRYSERVICEID="+serviceid+",ENTRYLOADID="+loadid+",ENTRYSOURCEID=0 WHERE ID="+Database.rowField(rowColumns, rowData, "ID"), null, null);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		if(callParams.get("LOADREQUESTTYPE").equals("1")){
			...
			if(callParams.get("CALLEXISTCOUNT").equals("0")){
				try{
					...
					Database.executeDBRequest(null, "FLATFILELEADSIMPORTER", "UPDATE <DBUSER>.[LEADSDATAFILEALLIASREQUESTCONTENT] SET ENTRYHANDLEFLAG=3,ENTRYSTATUS=2,ENTRYLASTACTIONDATETIME=GETDATE(),CALLREQUESTSTATUS=1,ENTRYSERVICEID=:SERVICEID,ENTRYLOADID=:LOADID,ENTRYSOURCEID=:SOURCEID,ENTRYLEADREQUESTTYPE="+_callparams.get("LOADREQUESTTYPE")+",ENTRYLEADREQUESTTYPEDESCRIPTION='ADDCALL' WHERE ID="+filecontententryid, _callparams, null);
				}
				catch(Exception e){
					e.printStackTrace();
				...
				this._recordProcessedFailed++;
				try {
					Database.executeDBRequest(null, "FLATFILELEADSIMPORTER", "UPDATE <DBUSER>.[LEADSDATAFILEALLIASREQUESTCONTENT] SET ENTRYHANDLEFLAG=4,ENTRYSTATUS=2,ENTRYLASTACTIONDATETIME=GETDATE(),CALLREQUESTSTATUS=2,ENTRYSERVICEID=:SERVICEID,ENTRYLOADID=:LOADID,ENTRYSOURCEID=:SOURCEID,ENTRYLEADREQUESTTYPE="+_callparams.get("LOADREQUESTTYPE")+",ENTRYLEADREQUESTTYPEDESCRIPTION='ADDCALL' WHERE ID="+filecontententryid, _callparams, null);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			...
			try{
				...
				Database.executeDBRequest(null, "FLATFILELEADSIMPORTER", "UPDATE <DBUSER>.[LEADSDATAFILEALLIASREQUESTCONTENT] SET ENTRYHANDLEFLAG=3,ENTRYSTATUS=3,ENTRYLASTACTIONDATETIME=GETDATE(),CALLREQUESTSTATUS=1,ENTRYSERVICEID=:SERVICEID,ENTRYLOADID=:LOADID,ENTRYSOURCEID=:SOURCEID,ENTRYLEADREQUESTTYPE="+_callparams.get("LOADREQUESTTYPE")+",ENTRYLEADREQUESTTYPEDESCRIPTION='REMOVECALL' WHERE ID="+filecontententryid, _callparams, null);
			}
			catch(Exception e){
				e.printStackTrace();

...
public class FlatFileRegisterRequest {
				...
				if(aliasFilesFound.length>0){
					...
					for(File alliasFile:aliasFilesFound){
						...
						if(alliasFile.exists()){
							...
							_newFileImportRequestSettings.put("ACTIVEREQUESTFILECOUNT","0");
							_newFileImportRequestSettings.put("INACTIVEREQUESTFILECOUNT","0");
							
							try {
								Database.executeDBRequest(null, "FLATFILELEADSIMPORTER","SELECT COUNT(*) AS ACTIVEREQUESTFILECOUNT FROM  <DBUSER>.[LEADSDATAFILEALLIASREQUEST] WHERE UPPER(REQUESTFILENAME)=UPPER(:REQUESTFILENAME) AND REQUESTFILEIMPORTACTIVE=1",_newFileImportRequestSettings,null);
							}
							catch(Exception e){
								//TODO
								e.printStackTrace();
							...
							try {
								Database.executeDBRequest(null, "FLATFILELEADSIMPORTER","SELECT COUNT(*) AS INACTIVEREQUESTFILECOUNT FROM  <DBUSER>.[LEADSDATAFILEALLIASREQUEST] WHERE UPPER(REQUESTFILENAME)=UPPER(:REQUESTFILENAME) AND REQUESTFILEIMPORTACTIVE=0",_newFileImportRequestSettings,null);
							}
							catch(Exception e){
								//TODO
								e.printStackTrace();
							...
							else if(!_newFileImportRequestSettings.get("ACTIVEREQUESTFILECOUNT").equals("0")){
								continue;
							}
							else if(!_newFileImportRequestSettings.get("INACTIVEREQUESTFILECOUNT").equals("0")){
								alliasFile.renameTo(new File((sourcePath=sourcePath.replaceAll("[\\\\]", "/"))+"handled/FAILED"+new SimpleDateFormat("yyyyMMddhhmmss").format(new Date())+"_"+alliasFile.getName()));
								continue;
							}
						}
					}
	...
	public void readRequestInfoData(Integer rowindex,ArrayList<String> rowData,ArrayList<String> rowColumns){
				...
				if(flatFileRequestContentImportSettings.get("REQUESTFILEIMPORTCONTENTSTATUS").equals("0")){
					...
					if(fileToAdd.exists()){
						...
						try{
							...
							fileContentImporter=new FlatFileRequestContentImporter(this,flatFileRequestContentImportSettings, fileToAdd);
						}
						catch(Exception e){
		...
		/*synchronized (_activeFlatFileRegisterRequestJobCalls) {
			if(!_activeFlatFileRegisterRequestJobCalls.containsKey(Database.rowField(rowColumns, rowData, "ID"))){
				...
				_activeFlatFileRegisterRequestJobCalls.put(Database.rowField(rowColumns, rowData, "ID"), fileCallsImporter=new FlatFileRequestCallsImporter(this,flatFileRequestCallImportSettings));
			}
		}*/
		if(fileToAdd!=null){
			synchronized (_activeFlatFileRegisterRequestFiles) {
		...
		if(fileContentImporter!=null){
			fileContentImporter.run();
		}
		/*if(fileCallsImporter!=null){
			fileCallsImporter.run();
		}*/
	}
	...
	public void flatFileRequestContentCompleted(
			...
			String requestid,File fileImported,String sourcePath) {
		synchronized (_activeFlatFileRegisterRequestJobFiles) {
			if(_activeFlatFileRegisterRequestJobFiles.containsKey(requestid)){
				if(_activeFlatFileRegisterRequestJobFiles.get(requestid).equals(fileImported)){
		...
		fileImported.renameTo(new File((sourcePath=sourcePath.replaceAll("[\\\\]", "/"))+"handled/PROCESSED"+new SimpleDateFormat("yyyyMMddhhmmss").format(new Date())+"_"+fileImported.getName()));
		flatFileRequestContentImporter=null;
	}

	public void flatFileRequestCallsCompleted(
			FlatFileRequestCallsImporter flatFileRequestCallsImporter,
			String requestid) {
		synchronized (_activeFlatFileRegisterRequestJobCalls) {
			if(_activeFlatFileRegisterRequestJobCalls.containsKey(requestid)){
				if(_activeFlatFileRegisterRequestJobCalls.get(requestid).equals(flatFileRequestCallsImporter)){
					_activeFlatFileRegisterRequestJobCalls.remove(requestid);
				}
			}
		}
		try {
			Database.executeDBRequest(null, "FLATFILELEADSIMPORTER", "UPDATE <DBUSER>.LEADSDATAFILEALLIASREQUEST SET REQUESTLASTHANDLEDATETIME=GETDATE(),REQUESTHANDLEFLAG=3,REQUESTFILEIMPORTACTIVE=0 WHERE REQUESTFILEIMPORTACTIVE=1 AND ID="+requestid,null, null);
		} catch (Exception e) {

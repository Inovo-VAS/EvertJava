				...
				while(maxRows>1){
					...
					if(_callParams.get("LOADREQUESTTYPE").equals("1")){
						...
						//if(!_callParams.get("METAFIELD1").equals("")){						
							try{
								Database.executeDBRequest(null,"FLATFILELEADSIMPORTER","SELECT COUNT(*) AS CALLEXISTCOUNT FROM <DBUSER>.DYNAMICCALLERLIST WHERE UPPER(ID)=UPPER(:SOURCEID) AND UPPER(SERVICEID)=UPPER(:SERVICEID) AND UPPER(LOADID)=UPPER(:LOADID)",_callParams,null);
							}
							catch(Exception e){
								e.printStackTrace();
							}

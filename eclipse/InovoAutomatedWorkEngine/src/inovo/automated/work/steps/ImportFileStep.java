		...
		if(currentFileFields!=null){
			if(currentFileFields.length>0){
				for(String currentfield:currentFileFields){
					...
					while(currentfield.startsWith("\"")) currentfield=currentfield.substring(1);
					while(currentfield.endsWith("\"")) currentfield=currentfield.substring(0,currentfield.length()-1);
			...
			File[]	filesFound=_fileDir.listFiles(new FilenameFilter() {
				...
				public boolean accept(File file, String name) {
					...
					for(int maskindex=0;maskindex<fileMask.length;maskindex++){
						...
						if(maskindex==0){
							if(name.startsWith(fsec)&&!fsec.equals("")){
								name=name.substring(fsec.length());
							}
						}
						else if(maskindex==fileMask.length-1){
							if(!name.endsWith(fsec)&&!fsec.equals("")){
								name="";
							}
						}
						else{
							if(name.indexOf(fsec)>-1){
								if(!fileMask[maskindex-1].equals("")){
									name=name.substring(name.indexOf(fsec)+fsec.length());
								}
								else{
									name=name.substring(0,name.indexOf(fsec))+name.substring(name.indexOf(fsec)+fsec.length());
								}
							}
							else{
								name="";
	...
	public void readImportFileData(Integer rowIndex,ArrayList<String> rowData,ArrayList<String> rowColumns){
		...
		if(rowIndex==0){
			...
			if(_fileimportrequestsettings.get("INLINESQLCOMMAND").equals("")){
				...
				String formatedInlineSqlCommand=this.formatInlineSQLCommand(_fileimportrequestsettings.get("INLINESQLCOMMAND"));
				try {
					_fileContentParams.put("EXPORTSTATUS", "2");
					_fileContentParams.put("EXPORTSTATUSDESCRIPTION", "");
					Database.executeDBRequest(null, "AUTOMATEDWORK", formatedInlineSqlCommand, _fileContentParams, null);
				} catch (Exception e) {
		...
		for(char isql:inlinesqlCommand.toCharArray()){
			if(readingParam){
				if(parameterName.length()>0&&endParamChar==']'&&isql==endParamChar){
					...
					else if(_fileContentParams.containsKey(parameterName.toUpperCase().trim())){
						this._inlineSqlCommand.append(":"+parameterName.toUpperCase().trim());
					}
					else{

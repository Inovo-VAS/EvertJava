...
public class ServiceLoads extends InovoHTMLPageWidget {
	...
	public void pageContent() throws Exception {
		...
		Database.executeDBRequest(pcoloadset, "PRESENCE", "SELECT RDATE AS LOADCREATIONDATETIME,SERVICEID,ISNULL((SELECT NAME FROM <DBUSER>.PCO_OUTBOUNDSERVICE WHERE PCO_OUTBOUNDSERVICE.ID=PCO_LOAD.SERVICEID),'') AS SERVICENAME, LOADID, DESCRIPTION AS LOADNAME,STATUS,(SELECT COUNT(*) AS CALLCOUNT FROM <DBUSER>.PCO_OUTBOUNDQUEUE WHERE PCO_LOAD.SERVICEID=PCO_OUTBOUNDQUEUE.SERVICEID AND PCO_LOAD.LOADID=PCO_OUTBOUNDQUEUE.LOADID) AS LEADCOUNT FROM <DBUSER>.PCO_LOAD ORDER BY PCO_LOAD.SERVICEID,PCO_LOAD.LOADID",null, null);

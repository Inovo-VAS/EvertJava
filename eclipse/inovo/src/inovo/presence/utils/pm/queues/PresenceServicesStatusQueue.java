...
public class PresenceServicesStatusQueue implements Runnable{
	...
	private class PresenceService implements Runnable{
		...
		private TreeMap<Integer,ArrayList<Object>> _serviceAgentsSet=null;
		private TreeMap<Integer,ArrayList<Object>> _serviceAgentCallsSet=null;
		private HashMap<String,Object> _serviceInfo=new HashMap<String,Object>();
		private HashMap<String,String> _serviceStatusFields=new HashMap<String,String>();
		
		private PresenceService(HashMap<String,Object> serviceInfo, ArrayList<String> additionalInfo,TreeMap<Integer,ArrayList<Object>> serviceAgentsSet,TreeMap<Integer,ArrayList<Object>> serviceAgentCallsSet){
		...
		public void run() {
			...
			try {
				...
				if(!this._serviceAgentsSet.isEmpty()){
					...
					HashMap<String,Object> serviceAgentInfo=new HashMap<String,Object>();
					...
					for(int serviceAgentsIndex:this._serviceAgentsSet.keySet()){
						...
						if(!serviceAgentInfo.get("LOGIN").equals("")){
							if(!_agents.contains(serviceAgentInfo.get("LOGIN").toString())) _agents.add(serviceAgentInfo.get("LOGIN").toString());
				...
				if(!this._serviceAgentCallsSet.isEmpty()){
					...
					HashMap<String,Object> serviceAgentCallInfo=new HashMap<String,Object>();
					...
					for(int serviceAgentCallIndex:this._serviceAgentCallsSet.keySet()){
						...
						this.setMaxTimeField(serviceAgentCallInfo.get("STATUS").toString(), serviceAgentCallInfo.get("TIME").toString());
						...
						//if(!serviceAgentCallInfo.get("LOGIN").equals("")){
							...
							if(!_agents.contains(serviceAgentCallInfo.get("LOGIN"))) _agents.add(serviceAgentCallInfo.get("LOGIN").toString());
			...
			if(this.canRunProcess()){
				try {
					...
					try {
						...
						TreeMap<Integer, ArrayList<Object>> listedServicesSet = PresenceServerConsole.listServices(this._presenceserverip);
						...
						if(listedServicesSet!=null){
							...
							for(int servicesrowindex:listedServicesSet.keySet()){
								...
								if(serviceData.get("STATUS").equals("ENABLED")){
									...
									TreeMap<Integer, ArrayList<Object>> listedServiceAgentsSet=PresenceServerConsole.listServiceAgents(serviceData.get("ID").toString(),this._presenceserverip,additionalServiceInfo);
									
									TreeMap<Integer, ArrayList<Object>> listedServiceAgentCallsSet=PresenceServerConsole.listServiceAgentCalls(serviceData.get("ID").toString(),this._presenceserverip,additionalServiceInfo);

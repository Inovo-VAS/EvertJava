...
public class LeadsAutomationNewCallsTask implements Runnable {
	...
	public LeadsAutomationNewCallsTask(HashMap<String,Object> callsTaskParams){
	...
	public void run() {
		...
		try{
			...
			_callsTaskParams.put("SUGGESTEDTOPCOUNT", "3000");
			...
			if(dynamiclist!=null){
				for(int rowindex:dynamiclist.keySet()){
					...
					LeadsAutomation.leadsAutomation("","").addCall(dyncalldata);
	...
	public void readRowData(Integer rowindex,ArrayList<String> rowData,ArrayList<String> rowColumns) throws Exception{
		...
		LeadsAutomation.leadsAutomation("","").addCall(_dyncalldata);

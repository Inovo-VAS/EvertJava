...
public class LeadsAutomationModifyCallsTask implements Runnable{
	...
	public LeadsAutomationModifyCallsTask(HashMap<String,Object> callsTaskParams){
		...
		try{
			...
			if(dynamiclist!=null){
				for(int rowindex:dynamiclist.keySet()){
					...
					LeadsAutomation.leadsAutomation("","").modifyCall(dyncalldata);
	...
	public void readRowData(Integer rowindex,ArrayList<String> rowData,ArrayList<String> rowColumns) throws Exception{
		...
		LeadsAutomation.leadsAutomation("","").modifyCall(_dyncalldata);

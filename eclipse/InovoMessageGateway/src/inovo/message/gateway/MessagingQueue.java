...
public class MessagingQueue {
	private static MessagingQueue _messagingQueue = null;
	    ...
	    /*try{
			  ...
			  for(String colName:messagingQueueSet.get((Integer)0)){
				  if(colName.startsWith("PARAM")&&colName.endsWith("NAME")){
					  ...
					  if(maxColSize<=currentColSize) maxColSize=currentColSize;
				  }
			  }
			  
			  _paramFieldCount=maxColSize;
		  }
		  catch (Exception e){
			  e.printStackTrace();
		  }*/
	    
	    if(_messagingQueue==null) _messagingQueue=new MessagingQueue();
	...
	public static int _paramFieldCount=50;
	...
	public boolean addNextMessageParams(HashMap<String,String> messageParams) throws Exception{
	  ...
	  private void processMessageParams(HashMap<String,String> messageparams) throws Exception
	  {
	    ...
	    try
	    {
	      ...
	      Database.executeDBRequest(null,"MESSAGEGATEWAY",paramInsertStatement,// "INSERT INTO <DBUSER>.[MESSAGE_GATEWAY_QUEUE] ([KEYPHRASE],[PHONENR],[EMAIL],[NAME],[IDNUMBER],[POSTALCODE],[SOURCE],[RECEIVEDDATETIME]"+paramInsertStatement+")", 
	    		  messageParamsProcessing,null);
	    		  //  this._messageParams);
	    }
	    catch (Exception e) {
	      e.printStackTrace();
	  ...
	  private void logKeyPhraseQueueRequest(String servicecallstatement, String defpriority, String defserviceid, String defloadid, HashMap<String, String> keyphraseidmap, HashMap<String, HashMap<String, String>> keyphrasesettingsmap, HashMap<String, String> loggedmessageentry)
	  {
	    ...
	    if(!noPhoneNrEntered&&!inValidPhoneNumbers){
		    ...
		    try
		    {
		    	...
		    	Database.executeDBRequest(null,"MESSAGEGATEWAY", sqlloggedmessagequeueupdate, loggedmessageentry,null);
		      
		    	Database.executeDBRequest(null,"MESSAGEGATEWAY", "SELECT [REQUESTSOURCEID],[REQUESTCALLPRIORITY] FROM <DBUSER>.[MESSAGE_GATEWAY_QUEUE] WHERE [ID]=:ID", loggedmessageentry,null);
		      if ((!((String)loggedmessageentry.get("REQUESTSOURCEID")).equals("0")) && (!((String)loggedmessageentry.get("REQUESTSOURCEID")).equals("")))
		        try {
		          ...
		          Database.executeDBRequest(null,"MESSAGEGATEWAY", "UPDATE <DBUSER>.[MESSAGE_GATEWAY_QUEUE] SET REQUESTHANDLEFLAG=:REQUESTHANDLEFLAG, REQUESTCREATESTATUS=:REQUESTCREATESTATUS, REQUESTCREATESTATUSOUTCOME=:REQUESTCREATESTATUSOUTCOME WHERE [ID]=:ID", 
		            loggedmessageentry,null);
		        }
		    } catch (Exception e) {
		      loggedmessageentry.put("REQUESTCREATESTATUS", "4");
		      loggedmessageentry.put("REQUESTHANDLEFLAG", "4");
		      ...
		      e.printStackTrace();
		      try {
					Database.executeDBRequest(null,"MESSAGEGATEWAY", "UPDATE <DBUSER>.[MESSAGE_GATEWAY_QUEUE] SET REQUESTHANDLEFLAG=:REQUESTHANDLEFLAG, REQUESTCREATESTATUS=:REQUESTCREATESTATUS, REQUESTCREATESTATUSOUTCOME=:REQUESTCREATESTATUSOUTCOME WHERE [ID]=:ID",loggedmessageentry,null);
				} catch (Exception er) {
					er.printStackTrace();
				}
		    }
	  ...
	  private void matchGatewayPhrases() throws Exception {
	    try {
	      Database.executeDBRequest(null,"MESSAGEGATEWAY", "UPDATE <DBUSER>.[MESSAGE_GATEWAY_QUEUE] SET [REQUESTHANDLEFLAG]=2 WHERE ISNULL([REQUESTHANDLEFLAG],1)<2", null,null);
	    } catch (Exception e) {
	      e.printStackTrace();
	      return;
	    }
	    ...
	    TreeMap<Integer,ArrayList<String>> loggedmessagesset =new TreeMap<Integer, ArrayList<String>>();
	    Database.executeDBRequest(loggedmessagesset,"MESSAGEGATEWAY", "SELECT TOP 1000 [ID],[KEYPHRASE],[PHONENR],[EMAIL],[NAME],[IDNUMBER],[POSTALCODE],[SOURCE],[RECEIVEDDATETIME]"+selectloggetMessagescustomcolumns+" FROM <DBUSER>.[MESSAGE_GATEWAY_QUEUE] WHERE [REQUESTHANDLEFLAG]=2 ORDER BY [RECEIVEDDATETIME] ASC", null,null);
	    if (loggedmessagesset == null) return;
	    if (loggedmessagesset.isEmpty()) return;
	    if (loggedmessagesset.size() > 1)
	    {
	      TreeMap<Integer,ArrayList<String>> keyphraseset =new TreeMap<Integer, ArrayList<String>>();
	      Database.executeDBRequest(keyphraseset, "MESSAGEGATEWAY", "SELECT [ID],UPPER([KEYPHRASE]) AS KEYPHRASE,[CREATEDATETIME],[MESSAGE_GATEWAY_KEYPHRASE_MAP].[SERVICEID] AS SERVICEID,[LOADID],[PARENTKEYPHRASEID],[PRIORITY],ISNULL(([ALTERNATEQUEUECALLSTATEMENT]),'') AS SERVICECALLSTATEMENT FROM <DBUSER>.[MESSAGE_GATEWAY_KEYPHRASE_MAP] LEFT JOIN <DBUSER>.[MESSAGE_GATEWAY_SERVICE_CALL_STMNT] ON [MESSAGE_GATEWAY_KEYPHRASE_MAP].[SERVICEID]=[MESSAGE_GATEWAY_SERVICE_CALL_STMNT].[SERVICEID] WHERE [MESSAGE_GATEWAY_KEYPHRASE_MAP].[ENABLEKEYPHRASE]='TRUE'", null,null);
	      ...
	      Database.cleanupDataset(keyphraseset);
	      keyphraseset=null;

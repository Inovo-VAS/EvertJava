...
public class Locking extends InovoHTMLPageWidget {
	...
	public void lockunlockclient() throws Exception{
		...
		this.endCell();
		this.startCell();
			this.fieldLabel("CIM USER");
		this.endCell();
		this.startCell();
			this.fieldInput("CIMUSER", "", "text", true);
		this.endCell();
		this.startCell();
			this.fieldLabel("FILE NAME");
		this.endCell();
		this.startCell();
			this.fieldInput("FILENAME", "", "text", true);
		this.endCell();
		this.startCell();
			this.action("SEARCH CLIENT", "searchlockunlockclient", "", "", "", "", "", "");
			this.action("RESET SEARCH CLIENT", "lockunlockclient", "", "", "lockunlockclient", "", "", "");
	...
	public void lockedclients() throws Exception{
		...
		if(!this.requestParameter("CIMUSER").equals("")){
			searchCriteria+=(searchCriteria.equals("")?"":" AND")+" UPPER(OCL.CIMUser) LIKE UPPER(:CIMUSER)+'%'";
			clientsSearchParams.put("CIMUSER", this.requestParameter("CIMUSER"));
		}
		
		if(!this.requestParameter("FILENAME").equals("")){
			searchCriteria+=(searchCriteria.equals("")?"":" AND")+" UPPER(OCL.FILENAME) LIKE UPPER(:FILENAME)+'%'";
			clientsSearchParams.put("FILENAME", this.requestParameter("FILENAME"));
		}
	...
	public void editaccountdata(boolean doneRecs,long rowindex,ArrayList<Object> data,ArrayList<Object> cols) throws Exception{
		if (rowindex>0){
			this._colIndex=0;
			for(Object dat:((rowindex==0)?cols:data)){
				if (rowindex==0){
	...
	public void lockunlockaccount() throws Exception{
		...
		this.startCell();
			...
			this.action("RESET SEARCH ACCOUNT", "lockunlockclient", "", "", "lockunlockaccount", "", "", "");
	...
	public void lockedaccounts() throws Exception{
		...
		if(!this.requestParameter("CIMUSER").equals("")){
			searchCriteria+=(searchCriteria.equals("")?"":" AND")+" UPPER(OCL.CIMUser) LIKE UPPER(:CIMUSER)+'%'";
			accountsSearchParams.put("CIMUSER", this.requestParameter("CIMUSER"));
		}
		
		if(!this.requestParameter("FILENAME").equals("")){
			searchCriteria+=(searchCriteria.equals("")?"":" AND")+" UPPER(OCL.FILENAME) LIKE UPPER(:FILENAME)+'%'";
			accountsSearchParams.put("FILENAME", this.requestParameter("FILENAME"));
		}
